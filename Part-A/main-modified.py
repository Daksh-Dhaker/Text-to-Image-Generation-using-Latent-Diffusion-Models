...
import train_clip
import train_dino
from dataset import CLEVRCaptionDataset_Aa, CLEVRProbeDataset, SimpleTokenizer, CLEVR_COLORS
...
def _get_or_extract(model_kind, ckpt_attr, tok_attr, task, split, args, device, loader):
    ...
    backbone = build_backbone(
        kind=model_kind, ckpt_path=ckpt_path,
        tokenizer_path=tok_path, max_len=args.max_len
    ).to(device)
    backbone.eval()
...
@torch.no_grad()
def predict_probe(probe, x, task: str):
    """Return per-sample predictions as a numpy array.

    For 'count'  : shape (N,)  – integer class indices.
    For 'colors' : shape (N, 8) – binary multi-hot array.
    """
    ...
    if task in {"count", "multiclass"}:
        return np.argmax(logits, axis=1)          # (N,) int

    # colors / multilabel
    probs = 1.0 / (1.0 + np.exp(-logits))
    return (probs >= 0.5).astype(np.int32)        # (N, 8) binary


def _build_prediction_json(filenames, preds, task: str) -> dict:
    """Convert a raw predictions array into the required dict format.

    colors → {"CLEVR_val_000000.png": ["brown", "gray"], ...}
    count  → {"CLEVR_val_000000.png": 5, ...}
    """
    out = {}
    for fname, pred in zip(filenames, preds):
        if task == "colors":
            out[fname] = [CLEVR_COLORS[i] for i, v in enumerate(pred) if v == 1]
        else:  # count
            out[fname] = int(pred)
    return out
...
def cmd_probe(args):
    ...
    for model_kind, ckpt_attr, tok_attr, display_name in _PROBE_MODELS:
        ...
        for task in ("count", "colors"):
            print(f"  task={task}")
            train_ds = CLEVRProbeDataset(
                root=args.probe_data_root, task=task, split="train", transform=transform)
            val_ds = CLEVRProbeDataset(
                root=args.probe_data_root, task=task, split="val", transform=transform)

            # Collect val filenames in dataset order (shuffle=False → order preserved)
            val_filenames = [fname for fname, _ in val_ds.samples]
            ...
            tr_cls, tr_gap, tr_y = _get_or_extract(
                model_kind, ckpt_attr, tok_attr, task, "train", args, device, train_loader)
            va_cls, va_gap, va_y = _get_or_extract(
                model_kind, ckpt_attr, tok_attr, task, "val",   args, device, val_loader)
            ...
            for emb_name, tr_x, va_x in (("cls", tr_cls, va_cls), ("gap", tr_gap, va_gap)):
                probe = LinearProbe(
                    input_dim=tr_x.shape[1], num_classes=num_classes).to(device)
                probe = train_probe(
                    probe, tr_x, tr_y, task=task,
                ...
                # ── Per-image predictions on the validation split ──────────────
                val_preds = predict_probe(probe, va_x, task=task)
                pred_dict = _build_prediction_json(val_filenames, val_preds, task)

                pred_path = out_dir / f"predictions_{model_kind}_{task}_{emb_name}.json"
                with open(pred_path, "w", encoding="utf-8") as fh:
                    json.dump(pred_dict, fh, indent=2)
                print(f"    [{emb_name}] predictions saved -> {pred_path}")
                # ──────────────────────────────────────────────────────────────

                torch.save(probe.state_dict(),
                           out_dir / f"probe_{model_kind}_{task}_{emb_name}.pt")
...
def cmd_tsne(args):
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    transform = build_probe_transform(224)
    count_ds = CLEVRProbeDataset(
        root=args.probe_data_root, task="count", split="train", transform=transform)
    ...
    for model_kind, ckpt_attr, tok_attr, title in tsne_cfgs:
        ...
        save_path = out_dir / f"tsne_{model_kind}.pdf"
...
def cmd_retrieval(args):
    ...
    val_ds = CLEVRCaptionDataset_Aa(
        root=args.probe_data_root, split="val",
...
def _add_eval_args(p):
    ...
    g.add_argument("--probe-data-root", default=None,
                   help="Root of Part Aa dataset  (required for probe/tsne)")
    g.add_argument("--data-root", default=None,
                   help="Root of Part A dataset   (required for retrieval)")
    ...
    g.add_argument("--max-len",    type=int, default=40,
                   help="Tokenizer context length (must match CLIP training)")
    g = p.add_argument_group("linear probe")
    g.add_argument("--probe-epochs", type=int,   default=200,
                   help="Epochs to train each linear probe")
    g.add_argument("--probe-lr",     type=float, default=1e-2,
                   help="AdamW learning rate for linear probes")
    g = p.add_argument_group("t-SNE")
    g.add_argument("--n-subsample", type=int, default=10000,
                   help="Points to subsample before running t-SNE")
    g = p.add_argument_group("retrieval")
    g.add_argument("--num-examples", type=int, default=10,
                   help="Number of example queries to save for the report")
    p.add_argument("--force-extract", action="store_true",
                   help="Ignore cached .npz feature files and re-extract")
...
def build_parser():
    ...
    subs.required = True
    subs.add_parser("train-clip",
                    help="Train CLIP  (pass --help after for full option list)",
                    add_help=False)
    subs.add_parser("train-dino",
                    help="Train DINO  (pass --help after for full option list)",
                    add_help=False)
    fmt = argparse.ArgumentDefaultsHelpFormatter
    for name, helpstr in (
        ("probe",     "Linear-probe all models -> Table 1"),
